...
import com.troy.streamingexchange.bitfinex.dto.marketdata.BitfinexTicker;
...
//    public static OrderStatusEnum adaptOrderStatus(BitfinexOrderStatusResponse order) {
...
//        } else {
//            return order.getExecutedAmount().compareTo(order.getOriginalAmount()) == 0 ? OrderStatusEnum.FILLED : null;
//        }
//    }
    ...
    public static OrderBook adaptOrderBook(BitfinexDepth btceDepth, CurrencyPair currencyPair) {
        OrdersContainer asksOrdersContainer = adaptOrders(btceDepth.getAsks(), currencyPair, OrderTypeEnum.ASK);
        OrdersContainer bidsOrdersContainer = adaptOrders(btceDepth.getBids(), currencyPair, OrderTypeEnum.BID);
    ...
    public static OrdersContainer adaptOrders(BitfinexLevel[] bitfinexLevels, CurrencyPair currencyPair, OrderTypeEnum orderType) {
    ...
    public static LimitOrder adaptOrder(BigDecimal originalAmount, BigDecimal price, CurrencyPair currencyPair, OrderTypeEnum orderType, Date timestamp) {
...
//        return new FixedRateLoanOrder(orderType, currency, amount, dayPeriod, id, (Date)null, rate);
//    }
//
...
//        OrderType orderType = direction.equalsIgnoreCase("loan") ? OrderType.BID : OrderType.ASK;
//        return new FloatingRateLoanOrder(orderType, currency, amount, dayPeriod, id, (Date)null, rate);
//    }

    public static Trade adaptTrade(BitfinexTrade trade, CurrencyPair currencyPair) {

        OrderTypeEnum orderType = trade.getType().equals("buy") ? OrderTypeEnum.BID : OrderTypeEnum.ASK;
...
//            OrderStatusEnum status = adaptOrderStatus(order);
...
//    }
    ...
    public static OrderBook adaptOrderBook(OrderBook orderBook) {
        //适应前端显示（卖盘高-低）
        Collections.reverse(orderBook.getAsks());
        return new OrderBook(new Date(), orderBook.getAsks(), orderBook.getBids());
    }
}
